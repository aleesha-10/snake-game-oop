    ...
    void move()
     {
        ...
        if (body[0].x >= SIZE) body[0].x = 0;
        else if (body[0].x < 0) body[0].x = SIZE - 1;
        else if (body[0].y >= SIZE) body[0].y = 0;
        else if (body[0].y < 0) body[0].y = SIZE - 1;
    }
    ...
    while (window.isOpen())
     {
        ...
        for (int i = 1; i < snake.size; ++i)
         {
            ...
            {
                cout << "Game Over! Self-collision." << std::endl;
                window.close();
            }
        }
        ...
        if (snake.body[0].x == snake.apple.x && snake.body[0].y == snake.apple.y) {
            ...
            eatSound.play();             // Play sound when eating apple
        }

        // Draw everything, including the border
        border.draw(window);  // Draw the border
        snake.draw(window, snakeShape);
        snake.apple.draw(window, appleShape);
        obstacles.Update();
        obstacles.Draw(window);

        window.draw(pointsText);
        livesText.setString("Lives: " + std::to_string(lives));  // Display lives
        window.draw(livesText);  // Draw lives text
